    ...
    return(
      <Fragment>
        <div className='linkWrapper'>
           <button className='btn-link' onClick={() => removeWebAddress(id)}>Remove</button>
           <button className='btn-link' onClick={onOpenModal}>Edit</button>
           <a target='blanck' href={`${link.link}`}>{link.link}</a>
        </div>
